...
const getRecentAssets = async (limit = 5) => {
  const assets = await getAssets();
  return assets.sort((a, b) => new Date(b.purchaseDate).getTime() - new Date(a.purchaseDate).getTime()).slice(0, limit);
};
const getRecentEmployees = async (limit = 5) => {
  const employees = await getEmployees();
  return employees.sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime()).slice(0, limit);
};
const IndexPage = () => {
            ...
            <Card className="h-full">
              ...
              <CardContent>
                <div className="space-y-3">
                  {assetStatusDistribution.map(item => <div key={item.status} className="flex items-center justify-between">
            ...
            <Card className="h-full">
              ...
              <CardContent>
                <ScrollArea className="h-[280px] pr-4">
                  <div className="space-y-4">
                    {recentAssets.map((asset: Asset) => <div key={asset.id} className="flex items-start space-x-3 pb-3 border-b last:border-0 last:pb-0">
                        <div className="mt-1">
                          <div className={cn("p-2 rounded-md", asset.type === "laptop" ? "bg-blue-100 text-blue-700" : asset.type === "smartphone" ? "bg-green-100 text-green-700" : asset.type === "tablet" ? "bg-purple-100 text-purple-700" : "bg-gray-100 text-gray-700")}>
                            <Package size={16} />
                          </div>
                        </div>
                    ...
                    {recentAssets.length === 0 && <p className="text-center text-muted-foreground py-4">Keine Assets gefunden</p>}
                  </div>
                </ScrollArea>
              </CardContent>
            </Card>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="col-span-1">
            <Card className="h-full">
              ...
              <CardContent>
                <ScrollArea className="h-[280px] pr-4">
                  <div className="space-y-4">
                    {recentEmployees.map(employee => <div key={employee.id} className="flex items-center space-x-3 pb-3 border-b last:border-0 last:pb-0">
                        <div className="flex-shrink-0">
                          {employee.imageUrl ? <div className="h-10 w-10 rounded-full overflow-hidden">
                              <img src={employee.imageUrl} alt={`${employee.firstName} ${employee.lastName}`} className="h-full w-full object-cover" />
                            </div> : <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center">
                    ...
                    {recentEmployees.length === 0 && <p className="text-center text-muted-foreground py-4">Keine neuen Mitarbeiter</p>}
                  </div>
                </ScrollArea>
              </CardContent>
            </Card>
