...
export const getAllBookings = async (): Promise<AssetBooking[]> => {
  try {
    const { data, error } = await safeTable('asset_bookings')
      .select('*')
...
export const isAssetAvailableForBooking = async (
  ...
  try {
    ...
    let query = safeTable('asset_bookings')
      .select('id')
      .eq('asset_id', assetId)
...
export const updateBookingStatuses = async (): Promise<void> => {
  try {
    ...
    await safeTable('asset_bookings')
...
export const getCurrentOrUpcomingBooking = async (assetId: string): Promise<AssetBooking | null> => {
  try {
    ...
    const { data: activeBookings, error: activeError } = await safeTable('asset_bookings')
      .select('*')
      .eq('asset_id', assetId)
      .eq('status', 'active')
      .lte('start_date', now)
    ...
    const { data: upcomingBookings, error: upcomingError } = await safeTable('asset_bookings')
      .select('*')
      .eq('asset_id', assetId)
      .eq('status', 'reserved')
      .gt('start_date', now)
